  ...
  const findPassengerAction = (passenger) => {
    const first = normalize(passenger.name).split(' ')[0];
    const candidates = clickableElements().map((el) => {
      ...
      const reviewContext = /avaliar|fazer avaliacao|deixar avaliacao|avalie sua experiencia/.test(`${text} ${parentText}`);
      let score = reviewContext ? 6 : -20;
      if (text.includes(normalize(passenger.name))) score += 8;
      else if (first && text.includes(first)) score += 4;
      if (parentText.includes(normalize(passenger.name))) score += 3;
      if (/excluir|cancelar|denunciar/.test(text)) score -= 20;
      return { el, score };
  ...
  const processPublication = async (force = false) => {
    ...
    if (!force && now() - state.publish.lastActionAt < 1200) return;
    ...
    if (hasSuccessMessage()) {
      ...
      saveState(); render();
      await sleep(700);
      if (!navigateToPassengerTrip(next)) pausePublication(`Não encontrei a viagem de ${next.name}.`);
      return;
    }
    ...
    if (field) {
      ...
      if (field.value !== passenger.suggestion) nativeSetValue(field, passenger.suggestion);
      const ratingClicked = clickRating(Number(passenger.rating));
      ...
      if (submit && !submit.disabled && submit.getAttribute('aria-disabled') !== 'true') {
        ...
        state.status = `Enviando avaliação de ${passenger.name}...`;
        saveState(); render();
        submit.click();
        return;
      }
      ...
      if (!ratingClicked && now() - state.publish.lastActionAt > 9000) {
        return pausePublication(`Não consegui definir a nota de ${passenger.name}.`);
      }
      state.publish.lastActionAt = now();
      saveState();
      return;
    ...
    const currentOfferId = offerIdFromUrl(location.href);
    if (currentOfferId !== passenger.offerId && passenger.offerUrl) {
      state.publish.lastActionAt = now();
      state.publish.stage = 'navigate-trip';
      state.status = `Abrindo a viagem de ${passenger.name}...`;
      saveState(); render();
      navigateToPassengerTrip(passenger);
      return;
    }
    ...
    state.publish.lastActionAt = now();
    saveState();
    if (state.publish.attempts >= 8) {
      pausePublication(`A tela de avaliação de ${passenger.name} não foi reconhecida. Abra manualmente essa avaliação e toque em “Retomar”.`);
    }
  };

  const clearQueue = () => {
    if (!confirm('Limpar toda a fila e o histórico local do RotaAi?')) return;
  ...
  const statusLabel = (status) => ({
    pendente: 'Pendente', analisando: 'Analisando', pronto: 'Pronto', aprovado: 'Aprovado', sem_perfil: 'Sem perfil',
  ...
  const renderPassengerCard = (passenger, index) => {
    ...
    return `<div class="rai-card ${passenger.published ? 'rai-published' : ''}" data-index="${index}">
      ...
      <textarea data-role="suggestion" ${disabled} placeholder="A avaliação aparecerá aqui.">${escapeHtml(passenger.suggestion || '')}</textarea>
      ...
      <div class="rai-row rai-card-actions">
        ${passenger.profileUrl ? '<button data-action="reanalyze">Reanalisar</button>' : ''}
        ${passenger.offerUrl ? '<button data-action="open-trip">Abrir viagem</button>' : ''}
      </div>
      ${passenger.reason ? `<div class="rai-note">${escapeHtml(passenger.reason)}</div>` : ''}
      ${passenger.reviews?.length ? `<details><summary>${passenger.reviews.length} avaliação(ões) usada(s) como base</summary><ol>${passenger.reviews.slice(0, 6).map((r) => `<li>${escapeHtml(r)}</li>`).join('')}</ol></details>` : ''}
    </div>`;
  };
  ...
  const addStyles = () => {
    ...
    style.textContent = `
      ...
      #${ROOT_ID} *{box-sizing:border-box}#${ROOT_ID} .rai-shell{pointer-events:auto;background:#fff;border-radius:18px 18px 0 0;box-shadow:0 -6px 30px #0004;max-height:82vh;display:flex;flex-direction:column}
      ...
      .rai-head{display:flex;align-items:center;gap:8px;padding:12px 14px;background:#006a6a;color:#fff;border-radius:18px 18px 0 0}.rai-head-title{font-weight:800;flex:1}.rai-head small{display:block;font-weight:400;opacity:.9}
      .rai-head button{background:#fff;color:#006a6a;border:0;border-radius:10px;padding:8px 10px;font-weight:800}.rai-body{overflow:auto;padding:10px}.rai-actions{display:grid;grid-template-columns:1fr 1fr;gap:7px}.rai-actions button{border:1px solid #006a6a;background:#fff;color:#006a6a;border-radius:10px;padding:10px 7px;font-weight:700}.rai-actions button.rai-primary{background:#006a6a;color:#fff}.rai-actions button.rai-danger{border-color:#a33;color:#a33}.rai-actions button:disabled{opacity:.45}
      .rai-status{margin:9px 0;padding:9px 10px;background:#e9f6f5;border-radius:10px}.rai-progress{font-weight:800}.rai-list{display:flex;flex-direction:column;gap:9px}.rai-card{border:1px solid #d7e4e4;border-radius:12px;padding:10px;background:#fff}.rai-published{background:#edf9ef}.rai-row{display:flex;align-items:center;gap:9px}.rai-between{justify-content:space-between}.rai-meta{font-size:11px;color:#5d7171;margin-top:2px}.rai-badge{font-size:11px;background:#e8eeee;border-radius:999px;padding:4px 7px;white-space:nowrap}.rai-card textarea{width:100%;min-height:76px;margin:8px 0 5px;border:1px solid #b8c9c9;border-radius:9px;padding:8px;font:inherit;resize:vertical}.rai-controls{justify-content:space-between;flex-wrap:wrap}.rai-controls select{padding:6px;border-radius:7px}.rai-approve{font-weight:700}.rai-card-actions{margin-top:7px}.rai-card-actions button{border:0;background:#edf4f4;color:#174f4f;border-radius:8px;padding:7px 9px}.rai-note{font-size:12px;color:#8b3b20;margin-top:7px}details{font-size:12px;margin-top:7px}ol{padding-left:20px}.rai-empty{text-align:center;padding:20px;color:#687}
  ...
  const bindEvents = (root) => {
    ...
    root.querySelector('[data-action="prepare"]')?.addEventListener('click', prepareAll);
    root.querySelector('[data-action="review"]')?.addEventListener('click', startReviewFlow);
    ...
    root.querySelector('[data-action="review-stop"]')?.addEventListener('click', () => pauseReviewFlow('Interrompida por você.'));
    ...
    root.querySelector('[data-action="reset-publish"]')?.addEventListener('click', resetPublication);
    ...
    root.querySelectorAll('[data-index]').forEach((card) => {
      ...
      card.querySelector('[data-action="open-trip"]')?.addEventListener('click', () => { location.href = passenger.offerUrl; });
      card.querySelector('[data-action="reanalyze"]')?.addEventListener('click', async () => {
        state.busy = true; render(); await analyzePassenger(passenger); state.busy = false; saveState(); render();
      });
    });
  };
  ...
  const render = () => {
    ...
    const cards = state.passengers.length ? state.passengers.map(renderPassengerCard).join('') : '<div class="rai-empty">Nenhum passageiro na fila.</div>';
    root.innerHTML = `<div class="rai-shell"><div class="rai-head"><div class="rai-head-title">RotaAi — Avaliações
      <small>${publish.active ? `Publicando ${Math.min(publish.index + 1, publish.queue.length)}/${publish.queue.length}` : state.review.active ? `Revisando ${Math.min(state.review.index + 1, state.review.queue.length)}/${state.review.queue.length}` : 'Preparar, revisar e publicar'}</small></div>
      <button data-action="toggle">${state.panelOpen ? 'Fechar' : 'Abrir'}</button></div>
      <div class="rai-body"><div class="rai-actions">
        <button class="rai-primary" data-action="prepare" ${state.busy || publish.active || state.review.active ? 'disabled' : ''}>1. Preparar pendentes</button>
        <button data-action="review" ${state.busy || publish.active || state.review.active || !state.passengers.some((p) => p.status === 'pronto' && p.suggestion) ? 'disabled' : ''}>2. Revisar uma por uma</button>
        ...
        ${state.review.active ? '<button class="rai-danger" data-action="review-stop">Pausar revisão</button>' : ''}
        <button class="rai-primary" data-action="publish" ${state.busy || publish.active || state.review.active || !state.passengers.some((p) => p.approved && !p.published) ? 'disabled' : ''}>3. Publicar aprovadas</button>
        ...
        ${publish.stage === 'done' || publish.paused ? '<button data-action="reset-publish">Encerrar publicação</button>' : ''}
        <button class="rai-danger" data-action="clear" ${publish.active || state.review.active ? 'disabled' : ''}>Limpar fila</button>
  ...
  const installNavigationHooks = () => {
    ...
    for (const method of ['pushState', 'replaceState']) {
      ...
      history[method] = function(...args) {
        ...
        setTimeout(() => { render(); processReviewFlow(true); processPublication(true); }, 500);
        return result;
      };
    }
    addEventListener('popstate', () => setTimeout(() => { render(); processReviewFlow(true); processPublication(true); }, 500));
  };
  ...
  const resume = () => {
    ...
    if (state.review.active) setTimeout(() => processReviewFlow(true), 700);
    if (state.publish.active) setTimeout(() => processPublication(true), 700);
  };

  window.RotaAiAndroid = { version: '0.3.0', openPanel, resume, scanTrips, prepareAll, processReviewFlow, processPublication };
  restoreState();
  installNavigationHooks();
  render();
  startTimer();
  if (state.review.active) setTimeout(() => processReviewFlow(true), 900);
  if (state.publish.active) setTimeout(() => processPublication(true), 900);
})();
